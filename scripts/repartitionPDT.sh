...
cd $COMPASS/compass
cmd="./run.sh $expclass 10000 -dataset $dataset -tattr trip_distance -pattr pickup_datetime -rpath $resultPath/csv -datatopic $datatopic -querytopic $qtopic -deltopic $deltopic -ticktopic $ttopic -k 128 -baselines pass -async -preload > $resultPath/log/$output &"
...
cmdfile=/tmp/pass/repartitionPDT
