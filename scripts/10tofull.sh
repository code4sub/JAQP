...
ttopic=ticktockpdt
deltopic=taxipdt-deletion
qtopic=taxipdt-query
dataset=taxipdt
datatopic="taxipdt-data-${sp}"

datasetpath=${COMPASS}/data/taxi_pickupdatetime_sorted_noheader.csv

querysize=2000

queryfile=${COMPASS}/files/TaxiPDT-RangeSumQuery-pickup_datetime-trip_distance-2000.txt
echo $datasetpath
...
cd $COMPASS/compass
cmd="./run.sh $expclass 13000 -dataset $dataset -tattr trip_distance -pattr pickup_datetime -rpath $resultPath/csv -datatopic $datatopic -querytopic $qtopic -deltopic $deltopic -ticktopic $ttopic -k 64 -baselines reservoir -async -preload &"
