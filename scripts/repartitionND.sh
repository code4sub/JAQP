...
# cmd="./run.sh $expclass 13000 -dataset $dataset -tattr volume -pattr open,close,low,high,idate -rpath $resultPath/csv -datatopic $datatopic -querytopic $qtopic -deltopic $deltopic -ticktopic $ttopic -k 256 -baselines pass -async -preload &"
echo $cmd
eval $cmd
cd -
...
cmdfile=/tmp/pass/repartitionND
